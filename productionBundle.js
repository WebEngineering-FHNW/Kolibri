 ...
 */
const getValue$4 = () => appenderArray;
 ...
 * @type { AppendCallback }
 */
const trace$4 = appenderCallback$1("trace")(console.trace);

/**
 * the function to append debug logs in this application
 * @type { AppendCallback }
 */
const debug$5 = appenderCallback$1("debug")(console.debug);

/**
 * the function to append info logs in this application
 * @type { AppendCallback }
 */
const info$4 = appenderCallback$1("info")(console.info);

/**
 * the function to append warn logs in this application
 * @type { AppendCallback }
 */
const warn$4 = appenderCallback$1("warn")(console.warn);

/**
 * the function to append error logs in this application
 * @type { AppendCallback }
 */
const error$4 = appenderCallback$1("error")(console.error);

/**
 * the function to append fatal logs in this application
 * @type { AppendCallback }
 */
const fatal$4 = appenderCallback$1("fatal")(console.error);/**
 ...
 * @type { AppendCallback }
 */
const trace$3 = appenderCallback(console.trace);

/**
 * the function to append debug logs in this application
 * @type { AppendCallback }
 */
const debug$4 = appenderCallback(console.debug);

/**
 * the function to append debug logs in this application
 * @type { AppendCallback }
 */
const info$3 = appenderCallback(console.info);

/**
 * the function to append warn logs in this application
 * @type { AppendCallback }
 */
const warn$3 = appenderCallback(console.warn);

/**
 * the function to append error logs in this application
 * @type { AppendCallback }
 */
const error$3 = appenderCallback(console.error);

/**
 * the function to append fatal logs in this application
 * @type { AppendCallback }
 */
const fatal$3 = appenderCallback(console.error);/**
...
loggerSuite.add("test log to multiple appender", assert => {
  const { appender }      = beforeStart();
  const countAppender     = Appender$1();
...
loggerSuite.run();const { trace: trace$2, debug: debug$3, info: info$2, warn: warn$2, error: error$2, fatal: fatal$2, getValue: getValue$2, reset: reset$2 } = Appender$3();

reset$2(); // clears the state of the appender

const msg1$1 = "Tobias Wyss";
...
arrayAppenderSuite.add("test add two values to array appender", assert => {
  const result1 = debug$3("first");
  const result2 = debug$3("second");
  assert.is(result1, T);
  assert.is(result2, T);
...
arrayAppenderSuite.add("test add all kind of levels to array appender", assert => {
  const resultTraceLog  = debug$3("debug");
...
arrayAppenderSuite.add("test default appender overflow implementation", assert => {
  ...
  const {trace, getValue, reset} = Appender$3(1);
  const result = trace(msg1$1);
  assert.is(getValue().length, 1);
  assert.is(result, T);
  const result2 = trace(msg1$1);
...
arrayAppenderSuite.add("test custom limit implementation", assert => {
  ...
  const {trace, reset} = Appender$3(1, onLimitReached);
  trace(msg1$1);
  trace(msg1$1);
  trace(msg2);
...
arrayAppenderSuite.add(
  ...
  assert => {
    const {trace, getValue, reset} = Appender$3(2, id);
    trace(msg1$1);
    trace(msg1$1);
...
arrayAppenderSuite.run();const { trace: trace$1, debug: debug$2, info: info$1, warn: warn$1, error: error$1, fatal: fatal$1 } = Appender();
...
consoleAppenderSuite.add("test add all kind of levels to console appender", assert => {
  const resultInfo   = info$1  ("info: expected log output from console appender: trace, debug, warn, error, fatal");
  const resultTrace  = trace$1 ("trace (expected)");
  const resultDebug  = debug$2 ("debug (expected)");
  const resultWarn   = warn$1  ("warn  (expected)");
  const resultError  = error$1 ("error (expected)");
  const resultFatal  = fatal$1 ("fatal (expected)");
...
consoleAppenderSuite.run();const { trace, debug: debug$1, info, warn, error, fatal, getValue: getValue$1, reset: reset$1 } = Appender$1();
reset$1(); // clear the state of the appender since it is a singleton

const countAppenderSuite = TestSuite("Count Appender");

countAppenderSuite.add("test add debug value to count appender", assert => {
  const result = debug$1("debug (expected)");
  assert.is(result, T);
  assert.is(getValue$1().debug, 1);
  reset$1();
});

countAppenderSuite.add("test add two values to count appender", assert => {
  const result1 = debug$1("first (expected)");
  const result2 = debug$1("second (expected)");
  assert.is(result1, T);
  assert.is(result2, T);
  assert.is(getValue$1().debug, 2);
  reset$1();
});
...
countAppenderSuite.add("test add all kind of levels to count appender", assert => {
  ...
  assert.is(getValue$1().trace,  1);
  assert.is(getValue$1().debug,  1);
  assert.is(getValue$1().info,   1);
  assert.is(getValue$1().warn,   1);
  assert.is(getValue$1().error,  1);
  assert.is(getValue$1().fatal,  1);
  reset$1();
  assert.isTrue(0 === getValue$1().trace);
  assert.isTrue(0 === getValue$1().debug);
  assert.isTrue(0 === getValue$1().info);
  assert.isTrue(0 === getValue$1().warn);
  assert.isTrue(0 === getValue$1().error);
  assert.isTrue(0 === getValue$1().fatal);
});

countAppenderSuite.run();const msg1 = "Some Log Message";

const observableAppenderSuite = TestSuite("Observable Appender");

const arrayAppender      = Appender$3();
let level;
let msg;
const observableAppender = Appender$2(arrayAppender)((newLevel, newMsg) => {
    level = newLevel;
    msg   = newMsg;
});
...
logUiControllerSuite.add("test setting log level as text", assert => {
  ...
  logUiController.loggingLevelController.setValue(LOG_INFO(name$1));
