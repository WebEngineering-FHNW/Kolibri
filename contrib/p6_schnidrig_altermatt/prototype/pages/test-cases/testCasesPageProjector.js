import { TestProjector } from "./testProjector.js";

export { TestCasesPageProjector }
 ...
 * @example
 * const testCasesPageController = PageController("test-cases", null);
 * testCasesPageController.setIconPath('./navigation/icons/house.svg');
 * TestCasesPageProjector(testCasesPageController, pinToContentElement, './pages/test-cases/test-cases.html');
 */
const TestCasesPageProjector = (pageController, pinToElement, contentFilePath) => {
    ...
    const initialize = () => {
        ...
        contentPromise.then(contentHtml => {
            ...
            const testDiv = TestProjector();
            const out = contentWrapper.querySelector('#out');
            if (null !== out) {
                out.append(...testDiv.children);
            }
