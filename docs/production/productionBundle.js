 ...
 * }
 */
const Appender$1 = appender => listener => (
  /** @type {AppenderType} */ {
 ...
 * @type { AppendCallback }
 */
const trace = appenderCallback$1("trace")(console.trace);

/**
 * the function to append debug logs in this application
 * @type { AppendCallback }
 */
const debug = appenderCallback$1("debug")(console.debug);

/**
 * the function to append info logs in this application
 * @type { AppendCallback }
 */
const info = appenderCallback$1("info")(console.info);

/**
 * the function to append warn logs in this application
 * @type { AppendCallback }
 */
const warn = appenderCallback$1("warn")(console.warn);

/**
 * the function to append error logs in this application
 * @type { AppendCallback }
 */
const error = appenderCallback$1("error")(console.error);

/**
 * the function to append fatal logs in this application
 * @type { AppendCallback }
 */
const fatal = appenderCallback$1("fatal")(console.error);/**
 ...
 */
const appenderCallback$2 = callback => msg => {
  callback(msg);
 ...
 * @type { AppendCallback }
 */
const trace$1 = appenderCallback$2(console.trace);

/**
 * the function to append debug logs in this application
 * @type { AppendCallback }
 */
const debug$1 = appenderCallback$2(console.debug);

/**
 * the function to append debug logs in this application
 * @type { AppendCallback }
 */
const info$1 = appenderCallback$2(console.info);

/**
 * the function to append warn logs in this application
 * @type { AppendCallback }
 */
const warn$1 = appenderCallback$2(console.warn);

/**
 * the function to append error logs in this application
 * @type { AppendCallback }
 */
const error$1 = appenderCallback$2(console.error);

/**
 * the function to append fatal logs in this application
 * @type { AppendCallback }
 */
const fatal$1 = appenderCallback$2(console.error);/**
...
const LogUiController = () => {
  ...
  const observableAppender = Appender$1
    ( Appender$2() )
 ...
 */
const [Test, name$1, logic] = Tuple(2);
    ...
    return {
        ...
        run:  () => {
            ...
            if (suiteAssert.results.every( id )) { // whole suite was ok, report whole suite
                ...
                tests.forEach( testInfo => test( testInfo(name$1), testInfo(logic) ) );
            }
        }
    };
};
 ...
 * @type { String }
 */
const clientId    = stamp() + stamp();const versionSuite = TestSuite("version");
 ...
 * See also {@link Pair}.
 */
const V = Pair;const churchSuite = TestSuite("church");
...
churchSuite.add("bool distr rule", assert => {
        ...
        const conjNF = a => b => c$$1 =>  beq (and (or(a)(b)) (or(a)(c$$1)))  (or (a) (and(b)(c$$1)));
        ...
        const disjNF = a => b => c$$1 =>  beq (or (and(a)(b)) (and(a)(c$$1)))  (and (a) (or(b)(c$$1)));
...
churchSuite.run();const churchNumberSuite = TestSuite("church numbers");
...
churchNumberSuite.run();const skiSuite = TestSuite("ski");
...
skiSuite.run();// noinspection PointlessArithmeticExpressionJS
...
arrayFunctionsSuite.run();// noinspection PointlessArithmeticExpressionJS
...
arraySuite.run();const domSuite = TestSuite("util/dom");
...
domSuite.run();const observableSuite = TestSuite("observable");
...
observableSuite.run();const pmSuite = TestSuite("presentationModel");
...
pmSuite.run();const logMessage  = "log message from loggerTest.js";
...
loggerSuite.add("test log to multiple appender", assert => {
  const { appender }      = beforeStart();
  const countAppender     = Appender$2();
...
loggerSuite.run();const { trace: trace$2, debug: debug$2, info: info$2, warn: warn$2, error: error$2, fatal: fatal$2, getValue: getValue$2, reset: reset$2 } = Appender();

reset$2(); // clears the state of the appender
...
arrayAppenderSuite.add("test add debug value to array appender", assert => {
  const result = debug$2("debug");
  assert.is(result, T);
  assert.is(getValue$2()[0], "debug");
  reset$2();
});

arrayAppenderSuite.add("test add two values to array appender", assert => {
  const result1 = debug$2("first");
  const result2 = debug$2("second");
  assert.is(result1, T);
  assert.is(result2, T);
  assert.is(getValue$2()[0], "first");
  assert.is(getValue$2()[1], "second");
  assert.is(getValue$2().length, 2);
  reset$2();
});
...
arrayAppenderSuite.add("test reset array appender", assert => {
  ...
  assert.is(getValue$2()[0], "first");
  reset$2();
  assert.isTrue(0 === getValue$2().length);
});

arrayAppenderSuite.add("test add all kind of levels to array appender", assert => {
  const resultTraceLog  = debug$2("debug");
  const resultDebugLog  = trace$2("trace");
  const resultInfoLog   = info$2("info");
  const resultWarnLog   = warn$2("warn");
  const resultErrorLog  = error$2("error");
  const resultFatalLog  = fatal$2("fatal");
  ...
  assert.is(getValue$2()[0], "debug");
  assert.is(getValue$2()[1], "trace");
  assert.is(getValue$2()[2], "info");
  assert.is(getValue$2()[3], "warn");
  assert.is(getValue$2()[4], "error");
  assert.is(getValue$2()[5], "fatal");
  reset$2();
  assert.isTrue(0 === getValue$2().length);
});
...
arrayAppenderSuite.add("test custom limit implementation", assert => {
  ...
  assert.is(getValue$2()[0], msg2);
  assert.is(getValue$2().length, 1);
  reset();
});
...
arrayAppenderSuite.run();const { trace: trace$3, debug: debug$3, info: info$3, warn: warn$3, error: error$3, fatal: fatal$3 } = Appender$3();
...
consoleAppenderSuite.add("test add all kind of levels to console appender", assert => {
  const resultInfo   = info$3  ("info: expected log output from console appender: trace, debug, warn, error, fatal");
  const resultTrace  = trace$3 ("trace (expected)");
  const resultDebug  = debug$3 ("debug (expected)");
  const resultWarn   = warn$3  ("warn  (expected)");
  const resultError  = error$3 ("error (expected)");
  const resultFatal  = fatal$3 ("fatal (expected)");
...
consoleAppenderSuite.run();const { trace: trace$4, debug: debug$4, info: info$4, warn: warn$4, error: error$4, fatal: fatal$4, getValue: getValue$3, reset: reset$3 } = Appender$2();
reset$3(); // clear the state of the appender since it is a singleton

const countAppenderSuite = TestSuite("Count Appender");

countAppenderSuite.add("test add debug value to count appender", assert => {
  const result = debug$4("debug (expected)");
  assert.is(result, T);
  assert.is(getValue$3().debug, 1);
  reset$3();
});

countAppenderSuite.add("test add two values to count appender", assert => {
  const result1 = debug$4("first (expected)");
  const result2 = debug$4("second (expected)");
  assert.is(result1, T);
  assert.is(result2, T);
  assert.is(getValue$3().debug, 2);
  reset$3();
});
...
countAppenderSuite.add("test reset count appender", assert => {
  ...
  assert.is(getValue$3().debug, 1);
  reset$3();
  assert.isTrue(0 === getValue$3().debug );
});

countAppenderSuite.add("test add all kind of levels to count appender", assert => {
  const traceResult  = trace$4("trace (expected)");
  const debugResult  = debug$4("debug (expected)");
  const infoResult   = info$4 ("info  (expected)");
  const warnResult   = warn$4 ("warn  (expected)");
  const errorResult  = error$4("error (expected)");
  const fatalResult  = fatal$4("fatal (expected)");
  ...
  assert.is(getValue$3().trace,  1);
  assert.is(getValue$3().debug,  1);
  assert.is(getValue$3().info,   1);
  assert.is(getValue$3().warn,   1);
  assert.is(getValue$3().error,  1);
  assert.is(getValue$3().fatal,  1);
  reset$3();
  assert.isTrue(0 === getValue$3().trace);
  assert.isTrue(0 === getValue$3().debug);
  assert.isTrue(0 === getValue$3().info);
  assert.isTrue(0 === getValue$3().warn);
  assert.isTrue(0 === getValue$3().error);
  assert.isTrue(0 === getValue$3().fatal);
});

countAppenderSuite.run();const msg1$1 = "Some Log Message";

const observableAppenderSuite = TestSuite("Observable Appender");

const arrayAppender      = Appender();
let level;
let msg;
const observableAppender = Appender$1(arrayAppender)((newLevel, newMsg) => {
    level = newLevel;
    msg   = newMsg;
});
const {debug: debug$5, getValue: getValue$4, reset: reset$4} = observableAppender;
...
observableAppenderSuite.add("Observable Appender setup", assert => {
    ...
    assert.is(getValue$4().length, 0);
});

observableAppenderSuite.run();const logUiControllerSuite = TestSuite("LogUiController");
...
dataflowSuite.run();const projectorUtilsSuite = TestSuite("projectorUtils");
...
projectorUtilsSuite.run();const simpleInputModelSuite = TestSuite("projector/simpleForm/simpleInputModel");
...
simpleInputModelSuite.run();const simpleInputControllerSuite = TestSuite("projector/simpleForm/simpleInputController");
...
simpleInputControllerSuite.run();const simpleFormControllerSuite = TestSuite("projector/simpleForm/simpleFormController");
...
simpleFormControllerSuite.run();const simpleInputProjectorSuite = TestSuite("projector/simpleForm/simpleInputProjector");
...
simpleInputProjectorSuite.add("binding", assert => {
    ...
    const [labelElement, spanElement] = projectChangeInput("TEST", controller, 0);
...
asyncTest("projector/simpleForm/debounce (async)", assert => {
    ...
    const [_, spanElement] = projectChangeInput("TEST", controller, debounceTimeMs);
    const inputElement = spanElement.querySelector("input");
...
simpleInputProjectorSuite.run();const simpleFormProjectorSuite = TestSuite("projector/simpleForm/simpleFormProjector");
...
simpleFormProjectorSuite.run();// production classes for bundling and statistics, without tests or examples